...
import Link from 'next/link';
import { useSession } from 'next-auth/react';
import { useChatStore } from '@/lib/hooks/singleStore'; // Import useChatStore
...
const ChatTemplate: React.FC<ChatTemplateProps> = ({ user, receiver, messages, onSendMessage }) => {
  const { data: session } = useSession();
  const selectedUser = useChatStore((state) => state.selectedUser); // Get selectedUser from store
  ...
  const handleAvatarClick = () => {
    if (selectedUser) {
      console.log(`THIS IS TEST FOR SELECTEDCONVID----> ${selectedUser._id}`);
    }
  };
